...
export const validateToken = async () => {
  ...
  export const createPaymentIntent = async (
    ...
    try {
      ...
      // Log the response status to identify the problem
      if (!response.ok) {
        const errorData = await response.json(); // Parse error response from server
        console.error('Error fetching payment intent:', errorData);  // Log the server error response
        throw new Error(`Error fetching payment intent: ${response.statusText}`);
      }
  
      return response.json();
    } catch (error) {
      console.error('Error in createPaymentIntent function:', error); // Log the caught error
